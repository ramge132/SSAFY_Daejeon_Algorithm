...
def mi_depth(current_id):
    global graph

    while True:

        graph[current_id][2] -= 1
...
def chk_create(current_id):
    ...
    while True:
        ...
        # print(parent_id, graph[parent_id][2], count)

        if graph[current_id][2] == 0:
            chk_flag = False
            break

        if graph[current_id][2] < count:
            chk_flag = False
            # print('chk')
            break
...
def create_node(content):
    ...
    if content[2] == -1:
        ...
        create_flag = chk_create(parent_id)

        # create_flag = count_depth(next_id, 2)
        # print(create_flag, content[2])
        ...
        if create_flag:
            ...
            # graph[content[1]].append(1)
            re_graph[content[2]].append(content[1])
            # plus_depth(next_id, 2)

def plus_depth(current_id, cnt):
    global graph

    parent_id = graph[current_id][0]

    graph[current_id][3] = max(graph[current_id][3], cnt)
    
    if parent_id == -1:
        return
    
    return plus_depth(parent_id, cnt + 1)
